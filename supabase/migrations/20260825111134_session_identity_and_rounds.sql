-- Additive rollout: authenticated identity, round membership and RPC mutations.
-- Apply together with the new frontend in a maintenance window. Legacy clients
-- fail closed for writes as soon as this migration is applied.

alter table public.sessions
  add column facilitator_user_id uuid references auth.users(id) on delete set null,
  add column create_request_id uuid;

create unique index sessions_facilitator_create_request_uidx
  on public.sessions (facilitator_user_id, create_request_id)
  where facilitator_user_id is not null and create_request_id is not null;
...
alter table public.sessions
  add constraint sessions_current_round_positive check (current_round >= 1);

alter table public.participants
  add column user_id uuid references auth.users(id) on delete set null,
  add column left_at timestamptz;

-- OPERATIONAL WARNING: this controlled cutover deliberately expires every
-- active legacy session. Legacy memberships have no auth identity and must
-- never be claimed by a new browser identity.
update public.sessions s
   set status = 'completed'
 where s.status = 'active'
   and (
     s.facilitator_user_id is null
     or exists (
...
alter table public.participants
  add constraint participants_name_valid
  check (char_length(btrim(name)) between 1 and 80) not valid;

alter table public.participants
  drop constraint unique_participant_name_per_session;
...
alter table public.votes
  add constraint votes_round_positive check (round >= 1),
  add constraint votes_session_participant_fkey
    foreign key (session_id, participant_id)
    references public.participants (session_id, id)
    on delete cascade
    not valid;

-- Vote DELETE payloads are not an authoritative client signal. Keep only the
-- primary key in old DELETE records so Postgres Changes cannot expose full rows.
...
create table public.round_participants (
  ...
  reestimate_used boolean not null default false,
  primary key (session_id, round, participant_id),
  constraint round_participants_session_participant_fkey
    foreign key (session_id, participant_id)
    references public.participants (session_id, id)
    on delete cascade
);

create index round_participants_participant_idx
  on public.round_participants (participant_id);

alter table public.round_participants enable row level security;
alter table public.round_participants replica identity full;
alter publication supabase_realtime add table public.round_participants;

-- Remove legacy direct mutations immediately. The SECURITY DEFINER RPCs below
-- are the only write API during both additive and lockdown phases.
...
revoke insert, update, delete on public.sessions from public, anon, authenticated;
revoke insert, update, delete on public.participants from public, anon, authenticated;
revoke insert, update, delete on public.votes from public, anon, authenticated;
revoke insert, update, delete on public.round_participants from public, anon, authenticated;
