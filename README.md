...
- Turnstile-verifisering bindes til Supabase-identiteten av Vercel-endepunktet og lagres som en 15-minutters databaseattest. Opprettelse og joining feiler lukket uten attest, også ved direkte RPC-kall. Vercel krever `SUPABASE_URL`, `SUPABASE_ANON_KEY`, `SUPABASE_SERVICE_ROLE_KEY`, `TURNSTILE_SECRET_KEY` og en matchende `VITE_TURNSTILE_SITE_KEY`.
